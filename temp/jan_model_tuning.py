# Here I tried to fit the flight data with a model predition by fitting a bunch of the 
# model constants. The residuals couldn't be reduced by adjusting the constants
#
...
full_params = np.zeros(14)
fitting_data = np.empty([len_used_data-1,44]) ## 44
...
q = ca.SX.sym('q', 4, 1)
w = ca.SX.sym('w', 3, 1)
parameters = ca.SX.sym('waypoint_voltage', 14)
...
# I_mat = ca.DM(np.array([
#             [parameters[5], parameters[8], parameters[9]],
#             [parameters[8],      parameters[6], parameters[10]],
#             [parameters[9], parameters[10], parameters[7]]
#         ]))
I_mat = ca.vertcat(
    ca.horzcat(parameters[5], parameters[8], parameters[9]),
    ca.horzcat(parameters[8], parameters[6], parameters[10]),
    ca.horzcat(parameters[9], parameters[10], parameters[7])
)
...
# [Ixx, Iyy, Izz, Ixy, Ixz, Iyz, lx, ly, lz]
theta_0 = np.array([0.0595, 0.0598, 0.0128, 0.0, 0.0003, 0.0010, 0.0015, 0.007, -0.209799])
...
lower = [
    0.01, 0.01, 0.005,   # inertia
   -0.01, -0.01, -0.01,  # cross inertia
   -0.1, -0.1, -0.3      # lever arm
]
...
# plt.plot(x_data, model(x_data, true_params), 'g-', label='True Curve')
# plt.plot(x_data, model(x_data, optimized_params), 'r--', label='Fitted Curve')
# plt.xlabel('x')
# plt.ylabel('y')
# plt.legend()
# plt.show()
...
#     params[i][3] = voltage[i]

#     local_params = np.concatenate((params[i], theta_0))

#     # predict the next state and compare with actual next state
#     x0 = state_data[i] + mc.dt* f(state_data[i],np.reshape(control_data[i], (4,1)), local_params)
#     flight_model_error[i] = state_data[i+1] -  np.reshape(x0, (13,))
#     model_error_sum += flight_model_error[i] @ flight_model_error[i]

# print("score: ", model_error_sum)
