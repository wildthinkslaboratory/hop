...
The drone's control loop runs at 50Hz on the Raspberry Pi 5, so computational efficiency is important.
...
Our drone has a total mass of 1.5 kg, and a diameter and height of 260mm and 500 mm respectively. It is powered by a 6S battery.
...
Communication between the controller on the Raspberry Pi and the Pixhawk is handled through ROS 2 running on Ubuntu 24.04 LTS. The Raspberry Pi sends actuator commands to the Pixhawk, and receives state estimation from it. This architecture allows the computationally expensive control algorithm to run on the Raspberry Pi while preserving the Pixhawk’s robust sensor fusion and hardware interfacing capabilities.
...
The system state $\vec{x}$ includes translational position $\vec{p} = [x, y,  z]$ in meters and velocity $\vec{v} = [v_x, v_y, v_z ]$ in meters per second both  in the world frame. The attitude represented as a unit quaternion $\vec{q} = [q_x, q_y,  q_z, q_w]$ and the angular velocity in the body frame $ \vec{\omega} = [\omega_x, \omega_y, \omega_z]$ in radians per second yeilding the full state vector $\vec{x} = [\vec{p}, \vec{v}, \vec{q},  \vec{\omega} ]$. The control vector  $\vec{u} =[ \theta_1, \theta_2, \pavg, \pdiff ]$ includes the two gimbal angles $\theta_1, \theta_2 $ in degrees, the average thrust between the two propellers $\pavg$ which takes values from $[0, 1]$ and  and the differential thrust between the propellers $\pdiff$ which  controls the rotation about the body $z$ axis and allows values from $[-1, 1]$
...
The actual control variables are defined as the average $\pavg=\frac{T_1+T_2}{2}$ and difference $\pdiff - T_1-T_2$ between the two stacked propeller thrust values $T_1$ and $T_2$. The command signal value for motors $T_1$ and $T_2$ is a value from $[0-1]$. 
