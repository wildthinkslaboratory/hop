...
\section{Introduction}	
	...
	There are a variety of ways to formulate an NMPC problem instances as NLPs. Different formulations  vary in their computation times and solution accuracy, particularly on embedded hardware platforms with limited computational resources.
...
The system state $\vec{x}$ includes translational position $\vec{p} = [x, y,  z]$ in meters and velocity $\vec{v} = [v_x, v_y, v_z ]$ in meters per second both  in the world frame. The attitude is represented as a unit quaternion $\vec{q} = [q_x, q_y,  q_z, q_w]$ and the angular velocity in the body frame $ \vec{\omega} = [\omega_x, \omega_y, \omega_z]$ is in radians per second yeilding the full state vector $\vec{x} = [\vec{p}, \vec{v}, \vec{q},  \vec{\omega} ]$. The control vector  $\vec{u} =[ \theta_1, \theta_2, \pavg, \pdiff ]$ includes the two gimbal angles $\theta_1, \theta_2 $ in degrees, the average thrust between the two propellers $\pavg$ which takes values from $[0, 1]$ and  and the differential thrust between the propellers $\pdiff$ which  controls the rotation about the body $z$ axis and allows values from $[-1, 1]$
