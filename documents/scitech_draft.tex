...
To meet our time requirement, we experimented with three formulations for the nonlinear programming problem (NLP):  a standard multiple shooting version, a Chebyshev pseudospectral collocation method and an orthogonal collocation method.  
...
\section{Introduction}	
	...
	There are a variety of ways to formulate an NMPC problem instances as an NLPs. Different formulations  vary in their computation times and solution accuracy, particularly on embedded hardware platforms with limited computational resources.
...
\begin{align}
	...
	&        0 \leq z.\nonumber
\end{align}
The problem instance enforces minimum and maximum gimbal angles of $\pm 20^{\circ}$ for the outer gimbal and $\pm 13.5^{\circ}$ for the inner gimbal, minimum and maximum thrust ranges, rate constraints for the servos and restrictons on the drone's vertical height.
...
\end{equation*}
prioritizes solutons with the final state in the horizon close  to the goal state.
...
\section{NLP Formulations}
	...
	Within each finite element, the differential equations $\dot{x} =  f(x(t),u(t))$ are approximated with a polynomial of order $K+1$ and interpolated over $K$ nodes within the interval.
