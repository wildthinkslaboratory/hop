...
\label{results}
	...
	The first case begins in the goal state and must simply hover to maintain the goal state. The second test state begins at  position of $\vec{p} = [1, 1, 0]$ and must move to the goal state. The third begins at position $\vec{p} = [1, 0.5, 1]$, has a $15^{\circ}$ vertical tilt, a slight rotation around the $z$ axis and some velocity in both the $x$ and $y$ directions.
